...
var masterA:Float = 11.25;
var masterL:Float = 1.0;
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Anga You Suck!', 0.2], //From 0% to 19%
		['Anga Society', 0.4], //From 20% to 39%
		['Anga Bad', 0.5], //From 40% to 49%
		['Anga Bruh', 0.6], //From 50% to 59%
		['Anga You can do better', 0.69], //From 60% to 68%
		['Anga Stoff Moment', 0.7], //69% (lenny)
		['Anga Good', 0.8], //From 70% to 79%
		['Anga Great', 0.9], //From 80% to 89%
		['Anga Sick!', 1], //From 90% to 99%
		['Anga Perfect FC', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var timeTxt:FlxText;
	var scoreTxtTween1:FlxTween; var scoreTxtTween2:FlxTween; var scoreTxtTween3:FlxTween;
	...
	override public function create()
	{
		...
		timeBar.alpha = 0; timeBar.y = -timeBar.height;
		...
		var botplayTexts:Array<String> = [
			"Showcase Mode", // This is the main one that's why it's at the top
			"Like \"introText.txt\"",
			"Like Geometry Dash's Loading Text Array",
			"Uhm... I love Dever",
			"Warning: 6th and under will insult you! This is the 5th"
		];
		var numBotTxt:Int = 5;

		botplayTxt = new FlxText(400, timeTxt.y + (timeTxt.height / 4) + 55, FlxG.width - 800, ClientPrefs.getGameplaySetting('randbotptxt') == true ? botplayTexts[Math.round(Math.random() * (numBotTxt - 1))] : "Showcase mode", 32);
		...
		botplayTxt.cameras = ClientPrefs.data.ghostTapping ? [camHUD] : [camGame];
	...
	public function updateScore(miss:Bool = false)
	{
		...
		+ ' | Miss To Hit Ratio: ' + '$songMisses/$songHits'
		+ ' | Rating: ' + str
		+ ' | ' + SONG.song
		+ ' | ' + Conductor.songPosition / 1000 + ' / ' + inst.length / 1000;

		if(ClientPrefs.data.scoreZoom && !miss && !cpuControlled)
		{
			if(scoreTxtTween1 != null) {
				scoreTxtTween1.cancel();
			}
			...
			scoreTxt.angle = Math.random() * masterA * 2 - masterA;
			scoreTxtTween1 = FlxTween.tween(scoreTxt.scale, {x: 1, y: 1}, masterL, {
			...
			scoreTxtTween2 = FlxTween.tween(scoreTxt, {angle: 0}, masterL, {
				ease: FlxEase.expoOut,
				onComplete: function(twn:FlxTween) {
					scoreTxtTween2 = null;
				}
		...
		if (miss) {
			if (scoreTxtTween3 != null) {
				scoreTxtTween3.cancel();
			}
			scoreTxtTween3 = FlxTween.tween(scoreTxt.scale, {x: 0.925, y: 0.925}, masterL, {
				ease: FlxEase.sineIn,
				onComplete: function(twn:FlxTween) {
					scoreTxtTween1 = null;
					scoreTxt.scale.x = 1;
	...
	function startSong():Void
	{
		...
		FlxTween.tween(timeBar, {y: timeTxt.y + (timeTxt.height / 4)}, 2, {ease: FlxEase.circOut});
		FlxTween.tween(timeTxt, {x: STRUM_X + (FlxG.width / 2) - 248}, 2, {ease: FlxEase.circOut});
	...
	override public function update(elapsed:Float)
	{
		...
		// angle stuff
		var mult:Float = FlxMath.lerp(1, iconP1.angle, FlxMath.bound(1 - (elapsed * 9 * playbackRate), 0, 1));
		iconP1.angle = mult;
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.angle, FlxMath.bound(1 - (elapsed * 9 * playbackRate), 0, 1));
		iconP2.angle = mult;
		iconP2.updateHitbox();

		var iconOffset:Int = 26;
		if (health > 2) health = 2;
		var hpBarcenter:Float;
		if (ClientPrefs.data.onlyIcons)
		{
			healthBar.alpha = 0;
		}
		else
		...
		hpBarcenter = (!ClientPrefs.data.onlyIcons) ? (healthBar.leftBar.x + healthBar.leftSize + healthBar.barOffset.x) : (healthBar.barWidth); // () cus haxe is retarted!
		iconP1.x = hpBarcenter + (150 * iconP1.scale.x - 150) / 2 - iconOffset;
		iconP2.x = hpBarcenter - (150 * iconP2.scale.x) / 2 - iconOffset * 2;
	...
	{
		if (ClientPrefs.data.hpDrain) {
			if (ClientPrefs.data.hpDrainStop) {
				if (health > ClientPrefs.data.hpDrainStopValue * 2)
				{
				health -= ClientPrefs.data.hpDrainAmount * 2;
				}
			}
			else
			{
	...
	override function beatHit()
	{
		...
		iconP1.angle = curBeat % 2 == 0 ? ClientPrefs.data.iconBobAngle : -ClientPrefs.data.iconBobAngle;
		iconP2.angle = curBeat % 2 == 1 ? ClientPrefs.data.iconBobAngle : -ClientPrefs.data.iconBobAngle;
