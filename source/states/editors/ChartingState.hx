...
class ChartingState extends MusicBeatState
{
	...
	[
		['null', "Nothing. Point Blank"],
		['Dadbattle Spotlight', "Used in Dad Battle (Ofc),\nValue 1: 0/1 = ON/OFF,\n2 = Target Dad\n3 = Target BF"],
		...
		['Change Character', "Value 1: Character to change (Dad, BF, GF)\nValue 2: New character's filename"],
		['Change Scroll Speed', "Value 1: Scroll Speed Multiplier (1 is default)\nValue 2: Time it takes to change fully in seconds."],
		['Set Property', "Set a PlayState property\nValue 1: Variable name\nValue 2: New value"],
	...
	**/
	...
	override function create()
	{
		...
		var tabs = [
			...
			{name: "Section", label: 'Bar'},
			{name: "Note", label: 'Note'},
			{name: "Events", label: 'Events'},
			{name: "Charting", label: 'Charting'},
			{name: "Data", label: 'Skins'},
		];
	...
	function addSongUI():Void
	{
		...
		var check_voices = new FlxUICheckBox(10, 25, null, null, "Has vocals", 100);
		...
		var saveButton:FlxButton = new FlxButton(110, 8, "Save (SPAM!)", function()
		{
			saveLevel();
		});
		...
		{
			openSubState(new Prompt('This action will clear current progress. And use "Reload Audio" for new chart\n\nProceed?', 0, function() {
				loadJson(_song.song.toLowerCase());
			},
			null, ignoreWarnings));
		});

		var loadAutosaveBtn:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 30, 'Load Autosave (If you forgot to save)', function()
		{
			PlayState.SONG = Song.parseJSONshit(FlxG.save.data.autosave);
			MusicBeatState.resetState();
		});

		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Mid-Song Events', function()
		{
		...
		var saveEvents:FlxButton = new FlxButton(110, reloadSongJson.y, 'Save Mid-Song Events', function ()
		{
			saveEvents();
		});

		var clear_events:FlxButton = new FlxButton(320, 310, 'Clear Mid-Song Events', function()
			{
				openSubState(new Prompt('This action will clear current MSE progress.\nPlease Save First!\nProceed?', 0, clearEvents, null,ignoreWarnings));
			});
		...
		var clear_notes:FlxButton = new FlxButton(320, clear_events.y + 30, 'Clear notes', function()
			{
				openSubState(new Prompt('This action will clear current note progress.\n\nProceed?', 0, function(){for (sec in 0..._song.notes.length) {
		...
		tab_group_song.add(new FlxText(stepperBPM.x, stepperBPM.y - 15, 0, 'Tempo:'));
		tab_group_song.add(new FlxText(stepperBPM.x + 100, stepperBPM.y - 15, 0, 'Offset:'));
	...
	function addSectionUI():Void
	{
		...
		check_mustHitSection = new FlxUICheckBox(10, 15, null, null, "Camera at P1", 100);
		check_mustHitSection.name = 'check_mustHit';
		check_mustHitSection.checked = _song.notes[curSec].mustHitSection;

		check_gfSection = new FlxUICheckBox(10, check_mustHitSection.y + 22, null, null, "GF Sings instead :)", 100);
		check_gfSection.name = 'check_gf';
		check_gfSection.checked = _song.notes[curSec].gfSection;
		// _song.needsVoices = check_mustHit.checked;

		check_altAnim = new FlxUICheckBox(check_gfSection.x + 120, check_gfSection.y, null, null, "Alt Animations", 100);
		...
		check_changeBPM = new FlxUICheckBox(10, stepperBeats.y + 30, null, null, 'New Tempo', 100);
		...
		var copyButton:FlxButton = new FlxButton(10, 190, "Ctrl+C on Bar", function()
		{
			notesCopied = [];
		...
		var pasteButton:FlxButton = new FlxButton(copyButton.x + 100, copyButton.y, "Ctrl+V To Bar", function()
		{
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Clear Bar", function()
		{
			if(check_notesSec.checked)
			{
		...
		check_notesSec = new FlxUICheckBox(10, clearSectionButton.y + 25, null, null, "Include Notes", 100);
		check_notesSec.checked = true;
		check_eventsSec = new FlxUICheckBox(check_notesSec.x + 100, check_notesSec.y, null, null, "Include Events", 100);
		check_eventsSec.checked = true;

		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap Notes", function()
		{
			for (i in 0..._song.notes[curSec].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Do a Duet", function()
		{
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Flip All Notes' Direction", function()
		{
			var duetNotes:Array<Array<Dynamic>> = [];
			for (note in _song.notes[curSec].sectionNotes)
			{
		...
		tab_group_section.add(new FlxText(stepperBeats.x, stepperBeats.y - 15, 0, 'Beats per Bar (KEEP AS 4!):'));
	...
	function addNoteUI():Void
	{
		...
		tab_group_note.add(new FlxText(10, 10, 0, 'DaLength:'));
	...
	function addEventsUI():Void
	{
		...
		var text:FlxText = new FlxText(20, 30, 0, "Event Selected:");
		...
		tab_group_event.add(text);
		value1InputText = new FlxUIInputText(20, 110, 100, lorem_ipsum ? "Lorem ipsum dolor sit amet" : "");
	...
	function changeEventSelected(change:Int = 0)
	{
		if(curSelectedNote != null && curSelectedNote[2] == null) //Is event note
		{
			...
			selectedEventText.text = 'Event Index: ' + (curEventSelected + 1) + ' / ' + curSelectedNote[1].length;
		}
		else
		{
			curEventSelected = 0;
			selectedEventText.text = 'Selected Event: null';
		}
		updateNoteUI();
	}
	...
	function addChartingUI() {
		...
		check_vortex = new FlxUICheckBox(10, 160, null, null, "Add Strums (Cosmetic, BETA)", 100);
		...
		check_warnings = new FlxUICheckBox(10, 120, null, null, "Don't bug me with progress warnings!", 100);
		...
		playSoundBf = new FlxUICheckBox(check_mute_inst.x, check_mute_vocals.y + 30, null, null, 'Play HitSound (Your notes)', 100,
			function() {
		...
		metronome = new FlxUICheckBox(10, 15, null, null, "Toggle Metronome", 100,
			function() {
		...
		disableAutoScrolling = new FlxUICheckBox(metronome.x + 120, metronome.y, null, null, "Disable Auto-Bar-Change (Not Recommended)", 120,
			function() {
		...
		sliderRate.nameLabel.text = 'Speedhack (in-editor)';
		tab_group_chart.add(sliderRate);
		#end

		tab_group_chart.add(new FlxText(metronomeStepper.x, metronomeStepper.y - 15, 0, 'Tempo:'));
		tab_group_chart.add(new FlxText(metronomeOffsetStepper.x, metronomeOffsetStepper.y - 15, 0,
		'smfO(:fse t)'));
		tab_group_chart.add(new FlxText(instVolume.x, instVolume.y - 15, 0, 'Instrumental Volume'));
	...
	function addDataUI()
	{
		...
		var check_disableNoteRGB:FlxUICheckBox = new FlxUICheckBox(10, 170, null, null, "No Note RGB", 100);
		...
		tab_group_data.add(new FlxText(gameOverCharacterInputText.x, gameOverCharacterInputText.y - 15, 0, 'Game Over Character Name(characters/):'));
